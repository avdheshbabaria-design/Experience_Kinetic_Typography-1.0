...
const DEFAULT_BASE_SETTINGS = {
  ...
  fxHighlight: false,
  bgMode: 'dark',
  manualTextColor: '#FFFFFF',
  manualShadowColor: '#FF00FF',
  manualShadowOpacity: 0.8,
...
const PRESET_LIBRARY = {
  ...
  'Comic Book': {
    fontMode: 'manual',
    manualFont: 'font-impact',
    textureOverlay: 'halftone',
    fxOutline: true,
    fxShadow: true,
    wordGrouping: 1,
    textScale: 1.5,
  ...
  'Cinematic Trailer': {
    ...
    cameraLens: '55mm',
    fontMode: 'manual',
    manualFont: 'font-bodoni',
    wordGrouping: 2,
    blurIntensity: 6,
    fadeRate: 0.1,
    textureOverlay: 'grunge',
  },
  'Vaporwave CRT': {
    bgMode: 'vaporwave',
    fontMode: 'manual',
    manualFont: 'font-vt323',
    neonGlow: true,
    glitchLevel: 4,
    momentumEnabled: true,
    fxHighlight: true,
  ...
  'Hacker Terminal': {
    bgMode: 'dark',
    fontMode: 'manual',
    manualFont: 'font-courier',
    wordGrouping: 6,
    textAlign: 'left',
    manualTextColor: '#39FF14',
    wordsPerLine: 6,
    textureOverlay: 'crt',
    overdriveLevel: 2,
  },
...
export default function App() {
  ...
  const [customPresets, setCustomPresets] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem('kinetic_presets')) || {};
    } catch (e) {
      return {};
    }
  });
  ...
  useEffect(() => {
    ...
    const primaryLang =
      settings.languages.find((l) => l !== 'en-US') || 'en-US';
    ...
    recognition.onerror = (e) => {
      if (e.error === 'not-allowed')
        showErrorToast('Microphone access blocked.');
      else if (e.error === 'network')
        showErrorToast('Network error: Speech API requires internet.');
      else if (e.error === 'language-not-supported')
        showErrorToast(
          `Language ${primaryLang} not supported. Install local OS keyboard.`
        );
  ...
  const handleRecordToggle = async () => {
    ...
    try {
      ...
      recorder.onstop = () => {
        ...
        a.href = url;
        a.download = `kinetic-mograph-${Date.now()}.webm`;
        a.click();
      ...
      if (
        ...
        showErrorToast(
          'Embedded view restricts built-in recording. Please use an external screen recorder (like OBS/QuickTime) along with the Chroma Key background mode.',
          8000
        );
      } else {
        showErrorToast(
          'Screen recording permission denied or not supported by browser.',
          6000
        );
      }
    }
  ...
  const processTextStream = (text) => {
    ...
    const getChunkSize = (index) => {
      ...
      return (
        Math.floor(
          (hash - Math.floor(hash)) * settingsRef.current.wordGrouping
        ) + 1
      );
    };
    ...
    chunks.forEach((chunkObj, index) => {
      if (!wordConfigs.current[index]) {
        const newWord = generateWordConfig(
          chunkObj.text,
          index,
          wordConfigs.current[index - 1]
        );
        ...
        if (settingsRef.current.iconEngine && Math.random() > 0.4) {
          ...
          const bgShape = {
            ...
            color:
              settingsRef.current.bgMode === 'vaporwave'
                ? COLORS_VAPOR[Math.floor(Math.random() * COLORS_VAPOR.length)]
                : COLORS_STD[Math.floor(Math.random() * COLORS_STD.length)],
        ...
        if (settingsRef.current.physicsEnabled) {
          for (let p = Math.max(0, index - 20); p < index; p++) {
            ...
            if (!oldWord) continue;
            const dx = oldWord.x - newWord.x;
            const dy = oldWord.y - newWord.y;
            const dist = Math.sqrt(dx * dx + dy * dy) || 1;
            const threshold = (newWord.size + oldWord.size) * 0.7;
            if (dist < threshold) {
              const pushForce =
                ((threshold - dist) / threshold) * (newWord.size * 1.2);
              oldWord.x += (dx / dist) * pushForce;
              oldWord.y += (dy / dist) * pushForce;
            }
          }
        }
  ...
  const generateWordConfig = (text, index, prevConfig) => {
    ...
    const baseFont =
      settingsRef.current.fontMode === 'manual'
    ...
    const size =
      ...
      settingsRef.current.textScale;
    const palette =
      settingsRef.current.bgMode === 'vaporwave' ? COLORS_VAPOR : COLORS_STD;
    ...
    const shadowOptions = [
      ...
      '15px 15px 0px rgba(230,57,70,0.8)',
      '-15px 15px 0px rgba(0,255,255,0.8)',
      '10px -10px 0px rgba(57,255,20,0.8)',
    ...
    if (prevConfig) {
      ...
      const wpl =
        settingsRef.current.wordsPerLine === 0
          ? getAutoWordsPerLine(settingsRef.current.aspectRatio)
          : settingsRef.current.wordsPerLine;
      ...
      switch (dir) {
        case 'right':
          x = prevConfig.x + estPrevWidth / 2 + estWidth / 2 + gap;
          y = prevConfig.y + (Math.random() * 60 - 30);
          break;
        case 'bottom':
          x = prevConfig.x + (Math.random() * 60 - 30);
          y = prevConfig.y + prevConfig.size / 2 + size / 2 + gap;
          break;
        case 'diagonal-down':
          x = prevConfig.x + estPrevWidth / 2 + gap;
          y = prevConfig.y + size / 2 + gap;
          break;
        case 'diagonal-up':
          x = prevConfig.x + estPrevWidth / 2 + gap;
          y = prevConfig.y - size / 2 - gap;
        ...
        case 'overlap':
        default:
          x = prevConfig.x + (Math.random() * 150 - 75);
          y = prevConfig.y + (Math.random() * 150 - 75);
          break;
      }
    }
  ...
  const updateCamera = (latestWord) => {
    ...
    const estHeight =
      lines.length * latestWord.size * settingsRef.current.lineSpacing;
    ...
    let camX = isNaN(latestWord.x)
      ? 0
      : -(latestWord.x + settingsRef.current.spawnAnchorX);
    let camY = isNaN(latestWord.y)
      ? 0
      : -(latestWord.y + settingsRef.current.spawnAnchorY);
    let camRotX = 0;
    let camRotY = 0;
    let camRotZ = isNaN(latestWord.rotation) ? 0 : -latestWord.rotation;

    if (settingsRef.current.cameraMode === 'manual') {
      camX += settingsRef.current.manualPanX;
      camY += settingsRef.current.manualPanY;
      camRotX += settingsRef.current.manualRotX;
      camRotY += settingsRef.current.manualRotY;
      camRotZ += settingsRef.current.manualRotZ;
    }
  ...
  const getAspectRatioStyle = () => {
    ...
    switch (settings.aspectRatio) {
      ...
      case 'cinema':
        return {
          ...baseStyle,
          width: 'min(100vw, 210vh)',
          height: 'min(100vh, 42.8vw)',
          border: '2px solid rgba(255,255,255,0.1)',
        };
      case 'imax':
        return {
          ...baseStyle,
          width: 'min(100vw, 143vh)',
          height: 'min(100vh, 69.9vw)',
          border: '2px solid rgba(255,255,255,0.1)',
        };
      case 'square':
        return {
  ...
  const wrapperBgColor = isChromaGreen
    ? '#00FF00'
    : isChromaBlue
    ? '#0000FF'
    : '#050505';
  ...
  const renderShape = (s) => {
    const style = {
      position: 'absolute',
      left: s.x + settings.spawnAnchorX,
      top: s.y + settings.spawnAnchorY,
  ...
  return (
    <div
      className={`relative w-full h-screen flex items-center justify-center overflow-hidden font-sans text-white select-none transition-colors duration-1000 ${
        !showUI && !showSettings ? 'cursor-none' : ''
      }`}
    ...
    >
      ...
      <div
        ...
        className={`${chaos.invert && !isChroma ? 'chaos-invert' : ''}`}
      >
        {/* Dynamic Backgrounds */}
        {!isChroma && (
          <>
            ...
            {settings.bgMode === 'vaporwave' && (
              <div className="vaporwave-grid" />
            )}
            {settings.bgMode === 'plasma' && <div className="plasma-bg" />}

            {/* Texture Overlays */}
            {settings.textureOverlay === 'halftone' && (
              <div className="texture-halftone" />
            )}
            {settings.textureOverlay === 'crt' && (
              <div className="texture-crt" />
            )}
            {settings.textureOverlay === 'grunge' && (
        ...
        <div
          className={`absolute inset-0 pointer-events-none z-40 ${
            chaos.vhs && !isChroma ? 'chaos-vhs bg-white/5' : ''
          }`}
        ></div>

        <div
          className={`camera-rig absolute ${
            chaos.rgb && !isChroma ? 'chaos-rgb' : ''
          }`}
          style={{
            transform: `scale(${safeScale}) rotateX(${safeRotX}deg) rotateY(${safeRotY}deg) rotateZ(${safeRotZ}deg) translate(${
              safeX + chaos.shakeX
            }px, ${safeY + chaos.shakeY}px)`,
          }}
        >
          ...
          {words.map((w, idx) => {
            ...
            const targetOpacity = isLatest
              ? 1
              : Math.max(0, 1 - age * settings.fadeRate);
            ...
            const glitchStyle = isGlitching
              ? { animation: `glitch-anim ${1 + w.glitchSeed}s infinite` }
              : {};
            const momentumStyle = settings.momentumEnabled
              ? {
                  animation: `float-parallax ${w.floatDuration} ease-in-out ${w.floatDelay} infinite`,
                }
              : {};
            const markerClass =
              settings.fxHighlight && w.hlOp > 0.4 && !isChroma
                ? 'text-highlight-fx'
                : '';

            const isInfinity = settings.wordGrouping === 6;
            const finalColor =
              isInfinity && !w.isOutline ? settings.manualTextColor : w.color;
            ...
            if (settings.fxShadow) {
              if (isInfinity) {
                ...
                finalShadow = w.shadowStr;
              }
            }

            const displayWpl =
              settings.wordsPerLine === 0
                ? getAutoWordsPerLine(settings.aspectRatio)
            ...
            return (
              <div
                key={w.id}
                className={`kinetic-word ${w.font} ${
                  w.isOutline ? 'text-outline-fx' : ''
                } ${settings.strobeFlicker && !isChroma ? 'flicker-fx' : ''}`}
                style={{
                  fontSize: `${w.size}px`,
                  color: finalColor,
                  left: `${w.x + settings.spawnAnchorX}px`,
                  top: `${w.y + settings.spawnAnchorY}px`,
                  transform: `translate(-50%, -50%) rotate(${w.rotation}deg)`,
                  zIndex: isLatest ? 50 : 10,
                  animationDelay: `${w.flickerDelay}s`,
                  textAlign: settings.textAlign,
                  whiteSpace:
                    settings.textAlign === 'justify' ? 'normal' : 'pre',
                  lineHeight: settings.lineSpacing,
                  '--outline-color': w.outlineColor,
                  '--outline-width': w.outlineWidth,
                  '--hl-color': w.hlColor,
                  '--hl-rot': w.hlRot,
                  '--hl-sx': w.hlSx,
                  '--hl-sy': w.hlSy,
      ...
      <div
        className={`absolute right-6 top-1/2 transform -translate-y-1/2 bg-black/70 backdrop-blur-3xl border border-white/10 pt-12 p-6 rounded-3xl w-80 z-[60] flex flex-col gap-6 shadow-2xl transition-all duration-500 max-h-[90vh] overflow-y-auto ${
          showSettings
      ...
      >
        <button
          ...
          className="absolute top-4 left-4 p-1.5 bg-white/10 hover:bg-white/20 rounded-full transition-colors"
        >
          <X className="w-5 h-5 text-white" />
        </button>

        <div className="bg-blue-900/40 border border-blue-500/30 rounded-lg p-2 text-center text-[9px] font-bold text-blue-200 tracking-wider uppercase mb-[-10px]">
          Tip: Ctrl+Click any slider to reset
        </div>

        {/* --- PRESET MANAGER --- */}
        <div className="flex flex-col gap-2 bg-white/5 p-3 rounded-xl border border-white/10">
          <label className="text-xs opacity-80 font-bold uppercase flex items-center text-yellow-400">
            <Bookmark className="w-4 h-4 mr-2" /> Preset Library
          </label>
          <div className="flex gap-2">
            ...
            >
              <optgroup label="Built-in Mograph">
                {Object.keys(PRESET_LIBRARY).map((p) => (
            ...
            <button
              ...
              className="bg-yellow-500/20 hover:bg-yellow-500/40 border border-yellow-500/50 p-2 rounded-lg text-yellow-400 transition-colors"
        ...
        <div className="flex flex-col gap-3">
          <label className="text-xs opacity-60 font-bold uppercase flex items-center">
            <Type className="w-4 h-4 mr-2" /> Typography Transform
          </label>
          ...
          <div className="flex flex-col gap-1">
            <div className="flex gap-2 mb-1">
              ...
              <button
                onClick={() => setSettings({ ...settings, fontMode: 'manual' })}
                className={`flex-1 py-1.5 text-[9px] font-bold uppercase rounded transition-all ${
            ...
            {settings.fontMode === 'manual' && (
              <select
                value={settings.manualFont}
                onChange={(e) =>
                  setSettings({ ...settings, manualFont: e.target.value })
                }
                className="w-full bg-black/50 border border-white/20 rounded text-[10px] p-1.5 text-white"
              >
                {FONTS_ALL.map((f) => (
                  <option key={f} value={f}>
                    {f.replace('font-', '').toUpperCase()}
                  </option>
                ))}
              </select>
            )}
          ...
          <div className="flex flex-col gap-1 mt-1">
            <div className="flex justify-between">
              <span className="text-[10px] opacity-60 uppercase font-bold text-green-400">
                Text Scale
              </span>
              <span className="text-[10px] opacity-60 font-bold text-green-400">
                {settings.textScale.toFixed(1)}x
              </span>
            </div>
            <input
              type="range"
              ...
              onClick={(e) => handleSliderReset(e, 'textScale', 1.0)}
          ...
          <div className="flex flex-col gap-1 mt-1">
            <div className="flex justify-between">
              ...
              <span className="text-[10px] opacity-60 font-bold text-green-400">
                {settings.wordSpacing.toFixed(1)}x
              </span>
            </div>
            <input
              type="range"
              ...
              onClick={(e) => handleSliderReset(e, 'wordSpacing', 1.0)}
              className="w-full accent-green-400"
            />
          </div>
          ...
          <div className="flex flex-col gap-1 mt-1">
            ...
            <input
              ...
              onClick={(e) => handleSliderReset(e, 'lineSpacing', 0.85)}
              className="w-full accent-green-400"
            />
          </div>
          ...
          <div className="flex flex-col gap-1 mt-1">
            <div className="flex justify-between">
              ...
              <span className="text-[10px] opacity-60 font-bold text-green-400">
                {settings.wordsPerLine === 0 ? 'Auto' : settings.wordsPerLine}
              </span>
            </div>
            <input
              type="range"
              ...
              onChange={(e) =>
                setSettings({
                  ...settings,
                  wordsPerLine: parseInt(e.target.value),
                })
              }
              onClick={(e) => handleSliderReset(e, 'wordsPerLine', 0)}
              className="w-full accent-green-400"
            />
          </div>

          <div className="flex flex-col gap-1 mt-1">
            <label className="text-[10px] opacity-60 font-bold uppercase">
              Alignment
            </label>
        ...
        <div className="flex flex-col gap-3 border-t border-white/10 pt-4">
          <label className="text-xs opacity-60 font-bold uppercase flex items-center">
            <Camera className="w-4 h-4 mr-2" /> Transform & 3D Camera
          </label>

          <div className="flex gap-2">
            <button
              onClick={() => setSettings({ ...settings, cameraMode: 'auto' })}
              className={`flex-1 py-1.5 text-[9px] font-bold uppercase rounded transition-all ${
            ...
            <button
              onClick={() => setSettings({ ...settings, cameraMode: 'manual' })}
              className={`flex-1 py-1.5 text-[9px] font-bold uppercase rounded transition-all ${
          ...
          {settings.cameraMode === 'manual' && (
            <div className="flex flex-col gap-2 p-3 bg-white/5 rounded-xl border border-white/10 mt-1">
              <div className="flex justify-between items-center mb-1">
                <span className="text-[10px] opacity-80 uppercase font-bold text-blue-400">
                  Relative Offsets
                </span>
                <button
                  onClick={resetManualCamera}
                  className="px-2 py-1 bg-white/10 hover:bg-white/20 rounded text-[8px] font-bold uppercase transition-colors text-white"
                >
                  Reset Center
                </button>
              </div>
              ...
              <input
                ...
                max="2000"
                step="50"
                value={settings.manualPanX}
                onChange={(e) =>
                  setSettings({
                    ...settings,
                    manualPanX: parseInt(e.target.value),
                  })
                }
                onClick={(e) => handleSliderReset(e, 'manualPanX', 0)}
              ...
              <input
                ...
                max="2000"
                step="50"
                value={settings.manualPanY}
                onChange={(e) =>
                  setSettings({
                    ...settings,
                    manualPanY: parseInt(e.target.value),
                  })
                }
                onClick={(e) => handleSliderReset(e, 'manualPanY', 0)}
                className="accent-white"
              />
              <div className="flex justify-between mt-1">
                <span className="text-[10px] opacity-60 uppercase font-bold">
              ...
              <input
                ...
                max="90"
                step="5"
                value={settings.manualRotX}
                onChange={(e) =>
                  setSettings({
                    ...settings,
                    manualRotX: parseInt(e.target.value),
                  })
                }
                onClick={(e) => handleSliderReset(e, 'manualRotX', 0)}
                className="accent-white"
              />
              <div className="flex justify-between mt-1">
                <span className="text-[10px] opacity-60 uppercase font-bold">
              ...
              <input
                ...
                max="90"
                step="5"
                value={settings.manualRotY}
                onChange={(e) =>
                  setSettings({
                    ...settings,
                    manualRotY: parseInt(e.target.value),
                  })
                }
                onClick={(e) => handleSliderReset(e, 'manualRotY', 0)}
                className="accent-white"
              />
              <div className="flex justify-between mt-1">
                <span className="text-[10px] opacity-60 uppercase font-bold">
              ...
              <input
                ...
                max="180"
                step="5"
                value={settings.manualRotZ}
                onChange={(e) =>
                  setSettings({
                    ...settings,
                    manualRotZ: parseInt(e.target.value),
                  })
                }
                onClick={(e) => handleSliderReset(e, 'manualRotZ', 0)}
                className="accent-white"
              />
            </div>
          )}
          ...
          <div className="flex flex-col gap-1 mt-1">
            ...
            <input
              ...
              max="1500"
              step="50"
              value={settings.spawnAnchorX}
              onChange={(e) =>
                setSettings({
                  ...settings,
                  spawnAnchorX: parseInt(e.target.value),
                })
              }
              onClick={(e) => handleSliderReset(e, 'spawnAnchorX', 0)}
              className="accent-white mb-1"
            />
            <input
              type="range"
              min="-1500"
              max="1500"
              step="50"
              value={settings.spawnAnchorY}
              onChange={(e) =>
                setSettings({
                  ...settings,
                  spawnAnchorY: parseInt(e.target.value),
                })
              }
              onClick={(e) => handleSliderReset(e, 'spawnAnchorY', 0)}
              className="accent-white"
            />
          </div>
            ...
            <div className="grid grid-cols-3 gap-1">
              {[
                ...
                <button
                  ...
                  className={`py-1.5 text-[9px] font-bold uppercase rounded transition-all ${
          ...
          <div className="flex flex-col gap-1 mt-1">
            ...
            <div className="grid grid-cols-3 gap-1">
              {Object.keys(LENS_OPTICS).map((lens) => (
                <button
                  key={lens}
                  onClick={() => setSettings({ ...settings, cameraLens: lens })}
                  className={`py-1.5 text-[9px] font-bold uppercase rounded transition-all ${
          ...
          <div className="flex flex-col gap-1 mt-1">
            <div className="flex justify-between">
              <span className="text-[10px] opacity-60 uppercase font-bold text-blue-400">
                Manual Zoom
              </span>
              <span className="text-[10px] opacity-60 font-bold text-blue-400">
                {settings.manualZoom.toFixed(1)}x
              </span>
            </div>
            <input
              type="range"
              ...
              onClick={(e) => handleSliderReset(e, 'manualZoom', 1.0)}
        ...
        <div className="flex flex-col gap-2 border-t border-white/10 pt-4">
          <label className="text-xs opacity-60 font-bold uppercase flex items-center">
            <MonitorPlay className="w-4 h-4 mr-2" /> Environment & Export
          </label>
          <div className="grid grid-cols-3 gap-1 mb-1">
            {['dark', 'vaporwave', 'plasma'].map((bg) => (
              <button
                key={bg}
                onClick={() => setSettings({ ...settings, bgMode: bg })}
          ...
          <div className="grid grid-cols-2 gap-1">
            <button
              onClick={() =>
                setSettings({ ...settings, bgMode: 'chroma-green' })
              }
              className={`py-1.5 text-[9px] font-bold uppercase rounded transition-all ${
                settings.bgMode === 'chroma-green'
            ...
            <button
              onClick={() =>
                setSettings({ ...settings, bgMode: 'chroma-blue' })
              }
              className={`py-1.5 text-[9px] font-bold uppercase rounded transition-all ${
                settings.bgMode === 'chroma-blue'
                  ? 'bg-[#0000FF] text-white shadow-[0_0_10px_#0000FF]'
                  : 'bg-white/10 hover:bg-white/20'
              }`}
            >
              Chroma Blue
          ...
          <div className="flex flex-col gap-1 mt-2">
            <label className="text-[10px] opacity-60 font-bold uppercase flex items-center">
              <Layers className="w-3 h-3 mr-1" /> Texture Overlay
            </label>
            <div className="grid grid-cols-4 gap-1">
              {['none', 'halftone', 'crt', 'grunge'].map((tex) => (
                <button
                  key={tex}
                  onClick={() =>
                    setSettings({ ...settings, textureOverlay: tex })
                  }
                  className={`py-1.5 text-[8px] font-bold uppercase rounded transition-all ${
                    settings.textureOverlay === tex
                      ? 'bg-white text-black'
                      : 'bg-white/10 hover:bg-white/20'
                  }`}
                >
        ...
        <div className="flex flex-col gap-3 border-t border-white/10 pt-4">
          <label className="text-xs opacity-60 font-bold uppercase flex items-center">
            <Zap className="w-4 h-4 mr-2" /> Kinetic FX
          </label>
          ...
          <div className="grid grid-cols-2 gap-2">
            <button
              ...
              className={`py-2 text-[10px] font-bold tracking-wider rounded-lg transition-colors border ${
            ...
            <button
              onClick={() =>
                setSettings({ ...settings, neonGlow: !settings.neonGlow })
              }
              className={`py-2 text-[10px] font-bold tracking-wider rounded-lg transition-colors border ${
                settings.neonGlow
            ...
            <button
              onClick={() =>
                setSettings({
                  ...settings,
                  strobeFlicker: !settings.strobeFlicker,
                })
              }
              className={`py-2 text-[10px] font-bold tracking-wider rounded-lg transition-colors border ${
                settings.strobeFlicker
            ...
            <button
              onClick={() =>
                setSettings({ ...settings, iconEngine: !settings.iconEngine })
              }
              className={`col-span-2 py-2 text-[10px] font-bold tracking-wider rounded-lg transition-colors border ${
                settings.iconEngine
          ...
          <div className="flex flex-col gap-1 mt-2">
            <div className="flex justify-between">
              <span className="text-[10px] opacity-60 uppercase font-bold text-red-400">
                Glitch Intensity
              </span>
              <span className="text-[10px] opacity-60 font-bold text-red-400">
                {settings.glitchLevel}
              </span>
            </div>
            <input
              type="range"
              ...
              onChange={(e) =>
                setSettings({
                  ...settings,
                  glitchLevel: parseInt(e.target.value),
                })
              }
              onClick={(e) => handleSliderReset(e, 'glitchLevel', 2)}
          ...
          <div className="flex flex-col gap-1 mt-2 p-3 bg-red-900/20 border border-red-500/30 rounded-xl">
            <div className="flex justify-between items-center mb-1">
              <span className="text-[10px] opacity-80 uppercase font-bold text-red-400">
                Chaos Overdrive
              </span>
              <span className="text-[10px] opacity-80 font-bold text-red-400">
                {settings.overdriveLevel === 0
                  ? 'OFF'
                  : settings.overdriveLevel}
              </span>
            </div>
            <input
              ...
              max="10"
              step="1"
              value={settings.overdriveLevel}
              onChange={(e) =>
                setSettings({
                  ...settings,
                  overdriveLevel: parseInt(e.target.value),
                })
              }
              onClick={(e) => handleSliderReset(e, 'overdriveLevel', 0)}
              className="w-full accent-red-500"
            />
            <span className="text-[8px] opacity-50 text-red-200 mt-1 uppercase text-center">
              Randomized Camera & Color FX Spikes
            </span>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-3 border-t border-white/10 pt-4">
          <label className="text-xs opacity-60 font-bold uppercase flex items-center">
            <Type className="w-4 h-4 mr-2" /> Pop-Art Text FX
          </label>
          <div className="grid grid-cols-3 gap-2">
            <button
              onClick={() =>
                setSettings({ ...settings, fxOutline: !settings.fxOutline })
              }
              className={`py-2 text-[10px] font-bold tracking-wider rounded-lg transition-colors border ${
                settings.fxOutline
                  ? 'bg-white text-black border-white'
            ...
            <button
              onClick={() =>
                setSettings({ ...settings, fxShadow: !settings.fxShadow })
              }
              className={`py-2 text-[10px] font-bold tracking-wider rounded-lg transition-colors border ${
                settings.fxShadow
                  ? 'bg-white text-black border-white'
            ...
            <button
              onClick={() =>
                setSettings({ ...settings, fxHighlight: !settings.fxHighlight })
              }
              className={`py-2 text-[10px] font-bold tracking-wider rounded-lg transition-colors border ${
                settings.fxHighlight
                  ? 'bg-white text-black border-white'
                  : 'border-white/20 text-white/50 hover:bg-white/10'
              }`}
            >
          ...
          <div className="flex gap-2">
            <button
              ...
              className={`flex-1 py-1.5 text-[10px] font-bold rounded-lg border ${
                settings.languages.includes('en-US')
                  ? 'bg-white text-black border-white'
                  : 'border-white/20 text-white/50 hover:bg-white/10'
              }`}
            >
            ...
            <button
              ...
              className={`flex-1 py-1.5 text-[10px] font-bold rounded-lg border ${
                settings.languages.includes('hi-IN')
                  ? 'bg-white text-black border-white'
                  : 'border-white/20 text-white/50 hover:bg-white/10'
              }`}
            >
            ...
            <button
              onClick={() => toggleLanguage('gu-IN')}
              className={`flex-1 py-1.5 text-[10px] font-bold rounded-lg border ${
                settings.languages.includes('gu-IN')
                  ? 'bg-white text-black border-white'
                  : 'border-white/20 text-white/50 hover:bg-white/10'
              }`}
            >
        ...
        <div className="flex flex-col gap-3 border-t border-white/10 pt-4">
          <label className="text-xs opacity-60 font-bold uppercase">
            Focus & Pacing
          </label>
          ...
          <div className="flex flex-col gap-1">
            <div className="flex justify-between">
              ...
              <span className="text-[10px] opacity-60 font-bold">
                {Math.round(settings.fadeRate * 100)}%
              </span>
            </div>
            <input
              type="range"
              ...
              onClick={(e) => handleSliderReset(e, 'fadeRate', 0.15)}
          ...
          <div className="flex flex-col gap-1">
            ...
            <input
              ...
              onClick={(e) => handleSliderReset(e, 'blurIntensity', 4)}
              className="w-full accent-white"
            />
          </div>
          ...
          <div className="flex flex-col gap-1">
            ...
            <input
              ...
              max="6"
              step="1"
              value={settings.wordGrouping}
              onChange={(e) =>
                setSettings({
                  ...settings,
                  wordGrouping: parseInt(e.target.value),
                })
              }
              onClick={(e) => handleSliderReset(e, 'wordGrouping', 1)}
              className="w-full accent-white"
            />
          </div>
          ...
          {settings.wordGrouping === 6 && (
            <div className="flex flex-col gap-2 p-3 bg-white/5 rounded-xl border border-white/10 mt-1">
              <span className="text-[10px] opacity-80 uppercase font-bold text-yellow-400 mb-1">
                Infinity Mode Styling
              </span>
              <div className="flex justify-between items-center">
                ...
                <input
                  type="color"
                  value={settings.manualTextColor}
                  onChange={(e) =>
                    setSettings({
                      ...settings,
                      manualTextColor: e.target.value,
                    })
                  }
              ...
              <div className="flex justify-between items-center">
                ...
                <input
                  type="color"
                  value={settings.manualShadowColor}
                  onChange={(e) =>
                    setSettings({
                      ...settings,
                      manualShadowColor: e.target.value,
                    })
                  }
                  className="w-6 h-6 rounded cursor-pointer bg-transparent border-none"
                />
              ...
              <div className="flex flex-col gap-1 mt-1">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-[10px] opacity-60 font-bold">
                    {settings.manualShadowOpacity.toFixed(1)}
                  </span>
                </div>
                <input
                  type="range"
                  ...
                  max="1"
                  step="0.1"
                  value={settings.manualShadowOpacity}
                  onChange={(e) =>
                    setSettings({
                      ...settings,
                      manualShadowOpacity: parseFloat(e.target.value),
                    })
                  }
                  onClick={(e) =>
                    handleSliderReset(e, 'manualShadowOpacity', 0.8)
                  }
      ...
      {/* UI Bottom Controls */}
      <div
        className={`absolute bottom-8 left-0 right-0 flex justify-center items-center space-x-6 z-50 transition-all duration-500 ${
          showUI || showSettings
      ...
      >
        <button
          ...
          title="Record Transparent Video"
          className={`flex items-center justify-center w-12 h-12 rounded-full backdrop-blur-md border transition-all ${
            isRecording
        ...
        <button
          ...
          className={`flex items-center justify-center w-16 h-16 rounded-full shadow-2xl transition-all duration-300 ${
            isListening
              ? 'bg-red-600 hover:bg-red-500 scale-110 animate-pulse text-white'
        ...
        <button
          ...
          className="flex items-center justify-center w-12 h-12 rounded-full bg-white/10 hover:bg-white/20 backdrop-blur-md border border-white/20 transition-all text-white"
        >
          {isFullscreen ? (
      ...
      {!isListening && words.length === 0 && !error && (
        <div className="absolute inset-0 flex flex-col items-center justify-center text-center pointer-events-none z-40">
          <h1 className="text-6xl md:text-8xl font-black font-montserrat tracking-tighter mb-4 opacity-80">
            KINETIC
            <br />
            VOICE
          </h1>
          <p className="text-xl font-bold font-inter tracking-widest uppercase opacity-50">
            Click Mic to Start Mograph
          </p>
        </div>
      )}
    </div>
